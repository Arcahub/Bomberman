...
Result Core::Create(ClientId clientId, std::uint64_t flags, Core** instance)
{
    ...
    auto result
        = DiscordCreate(DISCORD_VERSION, &params, &((*instance)->internal_));
    ...
    LogLevel minLevel, std::function<void(LogLevel, char const*)> hook)
{
    ...
    internal_->set_log_hook(
        internal_, static_cast<EDiscordLogLevel>(minLevel), &setLogHook_,
        wrapper);
}
...
discord::ApplicationManager& Core::ApplicationManager()
{
    if (!applicationManager_.internal_) {
        applicationManager_.internal_
            = internal_->get_application_manager(internal_);
    }
...
discord::RelationshipManager& Core::RelationshipManager()
{
    if (!relationshipManager_.internal_) {
        relationshipManager_.internal_
            = internal_->get_relationship_manager(internal_);
    }
...
discord::AchievementManager& Core::AchievementManager()
{
    if (!achievementManager_.internal_) {
        achievementManager_.internal_
            = internal_->get_achievement_manager(internal_);
    }
