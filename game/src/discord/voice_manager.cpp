...
Result VoiceManager::GetInputMode(InputMode* inputMode)
{
    ...
    auto result = internal_->get_input_mode(
        internal_, reinterpret_cast<DiscordInputMode*>(inputMode));
    return static_cast<Result>(result);
}

void VoiceManager::SetInputMode(
    InputMode inputMode, std::function<void(Result)> callback)
{
    static auto wrapper
    ...
    internal_->set_input_mode(
        internal_, *reinterpret_cast<DiscordInputMode const*>(&inputMode),
        cb.release(), wrapper);
}
...
Result VoiceManager::IsSelfMute(bool* mute)
{
    ...
    auto result
        = internal_->is_self_mute(internal_, reinterpret_cast<bool*>(mute));
    return static_cast<Result>(result);
}
...
Result VoiceManager::IsSelfDeaf(bool* deaf)
{
    ...
    auto result
        = internal_->is_self_deaf(internal_, reinterpret_cast<bool*>(deaf));
    return static_cast<Result>(result);
}
...
Result VoiceManager::IsLocalMute(Snowflake userId, bool* mute)
{
    ...
    auto result = internal_->is_local_mute(
        internal_, userId, reinterpret_cast<bool*>(mute));
    return static_cast<Result>(result);
}
...
Result VoiceManager::GetLocalVolume(Snowflake userId, std::uint8_t* volume)
{
    ...
    auto result = internal_->get_local_volume(
        internal_, userId, reinterpret_cast<uint8_t*>(volume));
    return static_cast<Result>(result);
}
