...
public:
    static void
    OnEntitlementCreate(void* callbackData, DiscordEntitlement* entitlement)
    {
        ...
        module.OnEntitlementCreate(
            *reinterpret_cast<Entitlement const*>(entitlement));
    }

    static void
    OnEntitlementDelete(void* callbackData, DiscordEntitlement* entitlement)
    {
        auto* core = reinterpret_cast<Core*>(callbackData);
        if (!core) {
            return;
...
Result StoreManager::GetSku(Snowflake skuId, Sku* sku)
{
    ...
    auto result = internal_->get_sku(
        internal_, skuId, reinterpret_cast<DiscordSku*>(sku));
...
Result StoreManager::GetSkuAt(std::int32_t index, Sku* sku)
{
    ...
    auto result = internal_->get_sku_at(
        internal_, index, reinterpret_cast<DiscordSku*>(sku));
    return static_cast<Result>(result);
}
...
Result
StoreManager::GetEntitlement(Snowflake entitlementId, Entitlement* entitlement)
{
    if (!entitlement) {
        return Result::InternalError;
    }
    ...
    auto result = internal_->get_entitlement(
        internal_, entitlementId,
        reinterpret_cast<DiscordEntitlement*>(entitlement));
    return static_cast<Result>(result);
}

Result
StoreManager::GetEntitlementAt(std::int32_t index, Entitlement* entitlement)
{
    if (!entitlement) {
        return Result::InternalError;
    }
...
Result StoreManager::HasSkuEntitlement(Snowflake skuId, bool* hasEntitlement)
{
    ...
    auto result = internal_->has_sku_entitlement(
        internal_, skuId, reinterpret_cast<bool*>(hasEntitlement));
    return static_cast<Result>(result);
}

void StoreManager::StartPurchase(
    Snowflake skuId, std::function<void(Result)> callback)
{
    static auto wrapper
        = [](void* callbackData, EDiscordResult result) -> void {
        std::unique_ptr<std::function<void(Result)>> cb(
