...
public:
    explicit RequestError(const char* str)
        : std::logic_error { str }
    {
    }
    explicit RequestError(const std::string& str)
        : std::logic_error { str }
    {
    }
};
...
public:
    explicit ResponseError(const char* str)
        : std::runtime_error { str }
    {
    }
    explicit ResponseError(const std::string& str)
        : std::runtime_error { str }
    {
    }
};
...
inline namespace detail {
        ...
        WinSock()
        {
            ...
            if (error != 0)
                throw std::system_error(
                    error, std::system_category(), "WSAStartup failed");
    ...
    public:
                ...
                getAddressFamily(internetProtocol), SOCK_STREAM, IPPROTO_TCP) }
        {
            ...
            const int value = 1;
            if (setsockopt(
                    endpoint, SOL_SOCKET, SO_NOSIGPIPE, &value, sizeof(value))
                == -1) {
                close();
                throw std::system_error(
                    errno, std::system_category(),
        ...
        void connect(
            const struct sockaddr* address, const socklen_t addressSize,
            const std::int64_t timeout)
        {
            ...
            if (result == -1) {
                if (WSAGetLastError() == WSAEWOULDBLOCK) {
                    ...
                    if (getsockopt(
                            endpoint, SOL_SOCKET, SO_ERROR, socketErrorPointer,
                            &optionLength)
                        == -1)
                        throw std::system_error(
                            WSAGetLastError(), std::system_category(),
                    ...
                    int socketError;
                    std::memcpy(
                        &socketError, socketErrorPointer,
                        sizeof(socketErrorPointer));

                    if (socketError != 0)
                        throw std::system_error(
                            socketError, std::system_category(),
                            "Failed to connect");
                } else
                    throw std::system_error(
                        WSAGetLastError(), std::system_category(),
            ...
            if (result == -1) {
                if (errno == EINPROGRESS) {
                    ...
                    if (getsockopt(
                            endpoint, SOL_SOCKET, SO_ERROR, &socketError,
                            &optionLength)
                        == -1)
                        throw std::system_error(
                            errno, std::system_category(),
                    ...
                    throw std::system_error(
                        errno, std::system_category(), "Failed to connect");
            }
#endif // _WIN32
        }
...
#ifdef _WIN32
            auto result = ::send(
                endpoint, reinterpret_cast<const char*>(buffer),
...
#else
            auto result = ::send(
                endpoint, reinterpret_cast<const char*>(buffer), length,
                noSignal);
        ...
        std::size_t
        recv(void* buffer, const std::size_t length, const std::int64_t timeout)
        {
            select(SelectType::read, timeout);
#ifdef _WIN32
            auto result = ::recv(
                endpoint, reinterpret_cast<char*>(buffer),
                static_cast<int>(length), 0);
...
#else
            auto result = ::recv(
                endpoint, reinterpret_cast<char*>(buffer), length, noSignal);

            while (result == -1 && errno == EINTR)
                result = ::recv(
                    endpoint, reinterpret_cast<char*>(buffer), length,
                    noSignal);

            if (result == -1)
                throw std::system_error(
                    errno, std::system_category(), "Failed to read data");
#endif // _WIN32
            return static_cast<std::size_t>(result);
        }
    ...
    private:
        ...
        void select(const SelectType type, const std::int64_t timeout)
        {
...
#ifdef _WIN32
            auto count = ::select(
                0, (type == SelectType::read) ? &descriptorSet : nullptr,
            ...
            if (count == -1)
                throw std::system_error(
                    errno, std::system_category(), "Failed to select socket");
            else if (count == 0)
                throw ResponseError("Request timed out");
#endif // _WIN32
        }
...
inline std::string urlEncode(const std::string& str)
{
    ...
    for (auto i = str.begin(); i != str.end(); ++i) {
        ...
        if ((cp >= 0x30 && cp <= 0x39) || // 0-9
            ...
            result += std::string("%") + hexChars[(*i & 0xF0) >> 4]
                + hexChars[*i & 0x0F];
...
public:
        ...
        = std::chrono::milliseconds { -1 })
    {
        ...
        for (const auto& parameter : parameters) {
            ...
            body += urlEncode(parameter.first) + "="
                + urlEncode(parameter.second);
        }
    ...
    {
        return send(
            method, std::vector<uint8_t>(body.begin(), body.end()), headers,
            timeout);
    }

    Response send(
        const std::string& method, const std::vector<uint8_t>& body,
        const std::vector<std::string>& headers,
        const std::chrono::milliseconds timeout
        = std::chrono::milliseconds { -1 })
    {
        ...
        std::unique_ptr<addrinfo, decltype(&freeaddrinfo)> addressInfo(
            info, freeaddrinfo);
        ...
        // send the request
        while (remaining > 0) {
            const auto size = socket.send(
                sendData, remaining,
                (timeout.count() >= 0) ? getRemainingMilliseconds(stopTime)
                                       : -1);
            remaining -= size;
            sendData += size;
        }
        ...
        for (;;) {
            ...
            responseData.insert(
                responseData.end(), tempBuffer.begin(),
                tempBuffer.begin() + size);

            if (state != State::body)
                for (;;) {
                    // RFC 7230, 3. Message Format
                    const auto i = std::search(
                        responseData.begin(), responseData.end(), crlf.begin(),
                        crlf.end());
                    ...
                    if (line.empty()) {
                        state = State::body;
                        break;
                    } else if (state == State::statusLine) // RFC 7230, 3.1.2.
                                                           // Status Line
                    {
                        state = State::headers;
                        ...
                        for (auto beginIterator = line.begin();
                             beginIterator != line.end();) {
                            const auto endIterator
                                = std::find(beginIterator, line.end(), ' ');
                            const std::string part { beginIterator,
                                                     endIterator };
                    ...
                    } else if (state == State::headers) // RFC 7230, 3.2. Header
                                                        // Fields
                    {
                        ...
                        headerValue.erase(
                            std::find_if(
                                headerValue.rbegin(), headerValue.rend(),
                                isNotWhiteSpace)
                                .base(),
                            headerValue.end());
            ...
            if (state == State::body) {
                // Content-Length must be ignored if Transfer-Encoding is
                // received
                if (chunkedResponse) {
                    for (;;) {
                        if (expectedChunkSize > 0) {
                            const auto toWrite
                                = (std::
                                       min)(expectedChunkSize, responseData.size());
                            response.body.insert(
                                response.body.end(), responseData.begin(),
                                responseData.begin()
                                    + static_cast<std::ptrdiff_t>(toWrite));
                            responseData.erase(
                                responseData.begin(),
                                responseData.begin()
                                    + static_cast<std::ptrdiff_t>(toWrite));
...
private:
    static std::int64_t
    getRemainingMilliseconds(const std::chrono::steady_clock::time_point time)
    {
